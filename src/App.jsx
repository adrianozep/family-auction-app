...
import { createHostAudioPlayer } from './audioPlayer.js'
import {
  collection,
  doc,
  getDoc,
...
export default function App() {
  ...
  const [useUploadedMusic, setUseUploadedMusic] = useState(true) // user gesture done
  ...
  const enableMusic = async () => {
    try {
      // Host: unlock audio (synth + MP3 + beeps)
      const s = synthRef.current
      const a = hostAudioRef.current
      if (a) a.setTrack(audioTrackForTheme(themeKey))
      if (s) await s.start()
      setMusicReady(true)
          ...
          <div style={{ width: '100%', maxWidth: 720 }}>
            <div className="controlsRow" style={{ gap: 14 }}>
              ...
              <div className="controlBox" style={{ minWidth: 0 }}>
                ...
                <div className="pill">
                  <span>Source</span>
                  <span style={{ opacity: .8 }}>{useUploadedMusic ? 'MP3' : 'Synth'}</span>
                </div>
                <div className="row">
                  <button onClick={() => setUseUploadedMusic(v => !v)}>
                    {useUploadedMusic ? 'Use Synth Instead' : 'Use Uploaded MP3s'}
                  </button>
                </div>
                <p className="small">MP3s live in <b>/public/audio/</b> (christmas.mp3, etc.).</p>
                <div className="row">

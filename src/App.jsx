...
export default function App() {
  ...
  // "First bid at current price" tracking (local demo only)
  // leadingBid = last successful bid overall (winner if round ends now)
  const [leadingBid, setLeadingBid] = useState(null) // { name, amount }
  // currentPriceHasBid = whether anyone has bid at *currentBid* yet
  const [currentPriceHasBid, setCurrentPriceHasBid] = useState(false)

  // Winner reveal (public) when round ends
  const [revealedWinner, setRevealedWinner] = useState(null) // { name, amount } | null

  // Player feedback messages
  const [globalNotice, setGlobalNotice] = useState('No bids yet')
  const [playerPrivateNotice, setPlayerPrivateNotice] = useState('')

  // keep bid input in sync
  useEffect(() => { setBidInput(currentBid) }, [currentBid])
  ...
  // when timer hits 0, reveal winner (if any leading bid)
  ...
  useEffect(() => {
    ...
    if (leadingBid) {
      setRevealedWinner(leadingBid)
    } else {
      setRevealedWinner({ name: 'No winner', amount: 0 })
    }
  ...
  const resetForNewPrice = (newPrice) => {
    // When host sets/raises bid, "no bids at this price yet"
    setCurrentBid(newPrice)
    setBidInput(newPrice)
    setCurrentPriceHasBid(false)
    setGlobalNotice(`No bids at $${newPrice} yet`)
    setRevealedWinner(null)
    // Keep leadingBid as-is so last bidder can still win if nobody bids at new price
  }

  const setMinAndReset = (nextMin) => {
    const m = Math.max(0, Number(nextMin) || 0)
    setMinBid(m)
    setMinBidInput(m)
    setLeadingBid(null)
    setCurrentPriceHasBid(false)
    setRevealedWinner(null)
    setGlobalNotice(`No bids at $${m} yet`)
    setCurrentBid(m)
    setBidInput(m)
  }

  const handlePlayerBid = () => {
    // Local-only behavior: the first click in THIS browser tab "wins" the current price.
  ...
  // PLAYER VIEW: ONLY current bid + timer + bid status message
  if (!isHost) {
    ...
    <div className="app">
      <div className="card">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
          ...
          {!started && <button onClick={() => { setStarted(true); setGlobalNotice(`No bids at $${currentBid} yet`) }}>Start Game</button>}
        </div>
        ...
        {started && (
          <>
            ...
            <div>
              ...
              <p className="small" style={{ marginTop: 8 }}>Time left: <b>{formatTime(timeLeft)}</b> {paused && timeLeft > 0 ? '(paused)' : ''}</p>
            ...
            <div className="controlsRow">
              ...
              <div className="controlBox">
                ...
                <div>
                  <p className="small">Minimum bid</p>
                  <input
                    type="number"
                    min={0}
                    value={minBidInput}
                  ...
                  <div className="row" style={{ marginTop: 6 }}>
                    <button onClick={() => setMinAndReset(minBidInput)}>Set Min</button>
                    <button onClick={() => resetForNewPrice(minBid)}>Reset (${minBid})</button>
                  </div>
                </div>

                <div>
                  <p className="small">Set current bid</p>
                  <input
                    type="number"
                    min={minBid}
                    value={bidInput}
                  ...
                  />
                  <div className="row" style={{ marginTop: 6 }}>
                    <button onClick={() => resetForNewPrice(Math.max(minBid, Number(bidInput) || 0))}>Set Bid</button>
                  </div>
                </div>
              </div>
              ...
              <div className="controlBox">
                ...
                <p className="small">Min bid: <b>${minBid}</b></p>

...
export default function App() {
  ...
  const playerBid = async () => {
    ...
    const lockedBidMessage = isGameHost ? '' : 'Wait for the next raise to bid again.'
    const baseStatusText = showWinner
      ...
      : room?.roundReady && !room?.started
        ...
        : room?.currentPriceHasBid
          ? lockedBidMessage
          : 'Waiting for bids'
    let statusText = baseStatusText === 'Waiting for bids' && (isGameHost || isMobile)
      ? ''
      : baseStatusText
    if (isMobile && !isGameHost && isLockedOutNotice) {
      statusText = ''
    }
